#!/usr/bin/env python3
"""
Test Phase 4 with specific restaurant names, budget 1000, rating 4.5
"""
...
def main():
    print("=== Phase 4 Test: With Restaurant Names ===\n")
    
    # Set up user preference with specific restaurant names
    ...
    print(f"  Restaurant Names: {user_pref.optional_constraints}")
    print()
    ...
    # Check if specified restaurants exist
    print("=== Checking Specified Restaurants ===")
    specified_restaurants = []
    for restaurant in restaurants:
        if restaurant.name in user_pref.optional_constraints:
            specified_restaurants.append(restaurant)
    ...
    if not specified_restaurants:
        print("❌ None of the specified restaurants found in dataset")
        return
    ...
    # Show candidates that match restaurant names
    print("=== Candidates Matching Restaurant Names ===")
    matching_candidates = []
    for restaurant in retrieval_result.candidate_set.candidates:
        if restaurant.name in user_pref.optional_constraints:
    ...
    try:
        ...
        for item in recommendation_result.items:
            ...
            is_requested = restaurant.name in user_pref.optional_constraints
            marker = "⭐" if is_requested else ""
            
            print(f"\n{item.rank}. {marker} {restaurant.name}")
            print(f"   Location: {restaurant.location or restaurant.city}")
            print(f"   Cuisines: {', '.join(restaurant.cuisines[:3])}")
            print(f"   Cost: {restaurant.cost_for_two}, Rating: {restaurant.rating}")
            print(f"   Explanation: {item.explanation}")
        
        # Show specifically requested restaurants in results
        requested_in_results = []
        for item in recommendation_result.items:
            restaurant = next(r for r in retrieval_result.candidate_set.candidates if r.id == item.restaurant_id)
            if restaurant.name in user_pref.optional_constraints:
                requested_in_results.append((item.rank, restaurant))
        
        print(f"\n=== Requested Restaurants in Results ===")
        if requested_in_results:
            for rank, restaurant in requested_in_results:
                print(f"⭐ Rank {rank}: {restaurant.name} (Cost: {restaurant.cost_for_two}, Rating: {restaurant.rating})")
        else:
            print("❌ None of the requested restaurants made it to the final recommendations")
